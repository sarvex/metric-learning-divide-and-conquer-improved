...
class SOProducts(BaseDataset):
    ...
    def __init__(self, root, classes, transform=None, albumentation=None):
        ...
        if classes.start in classes_train and classes.stop - 1 in classes_train:
            train = True

        if classes.start in classes_test and classes.stop - 1 in classes_test:
            train = False

        with open(os.path.join(root, f"Ebay_{'train' if train else 'test'}.txt")) as f:
